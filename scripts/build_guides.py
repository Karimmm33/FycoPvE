...
def main():
    ...
    crafts = json.load(open(os.path.join(HERE, "ref", "crafts.json"), encoding="utf-8"))
    spell_skill = {int(k): v[0] for k, v in crafts["spell_skill"].items()}
    per = build_spec_guides(cfg, item_names, item_spell, spell_enchant)
    # which profession casts each enchant spell, for "where to get it"
    extra = set()
    for d in per.values():
        for group in (d.get("enchants", []), d.get("gems", [])):
            for e in group:
                for o in e["options"]:
                    if o["kind"] == "spell":
                        o["craft"] = spell_skill.get(o["id"])
                    else:
                        extra.add(o["id"])
    write_spec_guides(per)
    # the enchant and gem items get full sources in Data/Items.lua: build_data.py reads this
    with open(os.path.join(HERE, "ref", "extra_items.json"), "w", encoding="utf-8") as f:
        json.dump(sorted(extra), f)
    print("%d enchant/gem items listed for build_data.py" % len(extra))
