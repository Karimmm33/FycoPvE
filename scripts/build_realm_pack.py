...
def main():
    ...
    # candidates: vendor items with stats, not already on any list, a caster could use
    on_lists = {e[0] for specs in bis.values() for phases in specs.values() for lists in phases.values()
                for lst in lists.values() for e in lst}
    offers = {}
    for vname, vitems in vendors.items():
        for iid, rec in (vitems or {}).items():
            iid = int(iid)
            slot = SLOT_OF.get(rec.get("loc") or "")
            if slot and iid not in on_lists and (rec.get("stats") or {}).get("ITEM_MOD_SPELL_POWER"):
                offers[iid] = (slot, vname, rec)
                items.setdefault(iid, rec)
    print("%d vendor items could go on a caster's lists" % len(offers))
    ...
    for (cls, spec), weights in WEIGHTS.items():
        for phase, slots in bis.get(cls, {}).get(spec, {}).items():
            for slot, lst in slots.items():
                cands = [i for i, (s, _, rec) in offers.items() if s == slot and usable_by_warlock(rec, slot)]
                if not cands:
                    continue
                ...
                if placed:
                    ...
                    for cid, pos, tier, beaten in placed:
                        ...
                        report.append("%s %s %s %s: %s (%.0f) at #%d, tier %d, above %s (%.0f)%s" % (
                            ...
                            "  [has Equip/Use/Set effects not weighed]" if items[cid].get("lines") else ""))
    ...
    for iid in sorted(used):
        ...
        entry = {"n": rec["n"], "q": rec.get("q") or 4, "lvl": rec.get("ilvl") or 0, "inv": INV_NUM[slot],
                 "lines": rec.get("lines") or None, "src": [src]}
